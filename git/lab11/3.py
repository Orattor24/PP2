...
def query_by_id(entry_id, new_name=None,new_surname = None ,new_number=None):
    ...
    try:
        with psycopg2.connect(**config) as conn:
            with conn.cursor() as cur:
                ...
                print(f"ID: {result[0]}, Name: {result[1]}, Surname: {result[2]} Number: {result[3]}")
        ...
        with psycopg2.connect(**config) as conn:
            with conn.cursor() as cur:
                ...
                if not result:
                    ...
                    new_surname = input("Введите новую фамилию (оставьте пустым, если не менять): ").strip()
